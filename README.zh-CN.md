...
PageCraft 0.4.1 把视觉意图一直带到源码修改环节：预览页面、点击元素或框选区域、记录当前响应式上下文，再把结构化工单交给负责代码的 Agent。新版增加后台完成自动同步、真实任务进度时间线和智能修改引导，并保留源码定位线索、截图上下文、视觉历史、安全恢复、主题和电影化动效工单；PageCraft 本身不会绕过 Agent 直接改写目标仓库。
...
- **后台完成自动同步**：Agent 执行期间可以关闭 PageCraft。重新打开后，PageCraft 会用持久化批次 ID、DSH 队列和已完成 turn 自动确认任务归属，刷新预览并生成修改前后对比；不再要求面板持续打开。
...
PageCraft 不要求在 Agent 执行期间一直保持打开。发送工单时会记录批次 ID 和当时的会话 turn 基线；重新打开面板后，它会核对以下事实：工单已经离开 DSH 队列、当前 Agent 已停止、会话中存在对应批次 ID，并且该请求之后出现了新的已完成 Agent turn。证据成立后，右侧会自动进入“完成同步”，刷新 iframe、捕获修改后截图并保存前后对比。

如果会话历史尚未加载、批次 ID 不可见或完成 turn 无法可靠归属，PageCraft 会继续保持等待状态，避免把其他普通任务误报为本批次完成。此时可点击进度卡中的**重新检测**；重复打开或重复检测不会为同一批次生成多份修改后截图。
...
- 自动同步依赖当前 DSH 会话窗口仍包含带批次 ID 的 PageCraft 请求及其完成 turn。极长会话发生历史窗口截断时会保守等待，可加载相关历史后点击“重新检测”。
