...
4. 点击 `选择元素` 可选择已有 DOM；点击 `框选区域` 可在希望新增内容的位置拖出矩形。
5. 松开鼠标后，蓝色选框会保留。拖动框内部可改变位置，拖动四条边或四个角上的八个控制点可调整大小，调整时仍会显示对齐参考线。
6. 绘制、移动或缩放时按住 `Alt` 可临时关闭吸附；绘制或拖动角点时按住 `Shift` 可锁定正方形。位置准确后点击 `确认选区`。
7. 在右侧写下修改要求，并加入评注队列。
8. 可以继续选择其他元素或区域；完成后点击 `发送给 Agent`。
9. Agent 修改源码后，点击刷新按钮查看结果并继续下一轮。
...
1. **客户端视图**：注册 `conversation.view`，提供地址栏、iframe、DOM/区域选择和评注队列。
2. **Host 预览路由**：获取目标 HTML，插入 `<base>` 和评注脚本，再返回给受控 iframe。脚本通过 `postMessage` 处理 DOM 命中、区域拖拽、对齐吸附、四点提取、链接和 GET 表单。
