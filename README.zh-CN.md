...
PageCraft 0.3.0 把视觉意图一直带到源码修改环节：预览页面、点击元素或框选区域、记录当前响应式上下文，再把结构化工单交给负责代码的 Agent。新版 Studio 工作台进一步加入源码定位线索、截图上下文、视觉历史、安全恢复、设计指导、主题和电影化动效工单；PageCraft 本身不会绕过 Agent 直接改写目标仓库。
...
4. 可按需捕获截图，写下一条或多条要求，加入队列后选择**发送给 Agent**。
5. PageCraft 保存修改前快照；Agent 完成后重新加载预览，在可捕获时保存修改后快照，并把该批次加入**比较与历史**。
